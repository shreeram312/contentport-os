  ...
  React.useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (isDragging.current) {
        ...
        // Invert the delta for right-side dragging
        const adjustedDeltaWidth = side === "right" ? -deltaWidth : deltaWidth;
        const newWidthPx = startWidth.current + adjustedDeltaWidth;
